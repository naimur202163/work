...
import { getKarmaSentByUserTransactions } from "../actions";
...
const KarmaSentTable = ({ userId, userrole }) => {
    ...
    if (userTransactions.karmaSent.length === 0) {
        return "No payment history yet!";
    }
    const transactions = userTransactions.karmaSent;
    return (
        <Wrapper>
            <TableContainer>
                <Table aria-label="collapsible table">
                    <TableHead>
                        <TableRow>
                            <TableCell className="text-color" align="center">
                                {userrole === 2 ? 'Video Karma Tip/Referral' : 'Video Karma Tip'}
                            </TableCell>
                            <TableCell className="text-color" align="center">
                                Amount
                            </TableCell>
                    ...
                    <TableBody>
                        {transactions &&
                            transactions.length && transactions.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                                <TableRow key={index + 1}>
                                    ...
                                    <TableCell className="text-color" align="center">
                                        <a
                                            href={`${process.env.REACT_APP_BASE_URL}channel/${getUserName(row)}`}
                                        >
                                            {getUserName(row)}
                                        </a>
                                    </TableCell>
