...
const AddMoreVideosModel = ({ open, closeHandler, seriesId }) => {
  ...
  // states
  const [tab, setTab] = useState("UPLOAD_VIDEOS"); // UPLOAD_VIDEOS, MANAGE_VIDEOS, CHOOSE_VIDEOS
  ...
  const uploadVideoHandler = async (idx, file) => {
    ...
    for (let index = 0; index < numberOfChunks; index++) {
      ...
      if (url && publicId) {
        ...
        setFileInfos((prevInfo) => [...prevInfo, obj]);
      }
    }
  };
  ...
  const handleIsChecked = (position, e, videoId) => {
    ...
    isChecked.forEach((item, i) => {
      ...
      if (i === position && e.target.checked === false) {
        dispatch(
          removeVideoFromSeries(seriesId, {
            videoId,
          })
        );
      }
  ...
  if (seriesId) {
    return (
      <>
        <AddMoreVideosModelWrapper>
          <div className="addMoreHeader">
            <div className="addMoreHeader__left">
              <CloseIcon
                onClick={() => {
                  if (fileInfos && fileInfos.length > 0) {
                    return toast.error(
              ...
              />
              <h1 className="addMoreHeader__title">Edit Videos ...</h1>
            </div>
            {tab === "MANAGE_VIDEOS" ? null : (
              <div
                ...
                className="addMoreHeader__right"
              >
                <Button>
                  {tab === "UPLOAD_VIDEOS"
                    ? `${
...
export default AddMoreVideosModel;
...
const AddMoreVideosModelWrapper = styled.div`
  position: fixed;
  left: 0;
  right: 0;
  margin: auto;
