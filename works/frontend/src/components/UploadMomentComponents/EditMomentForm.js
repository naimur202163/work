...
const EditMomentForm = ({ close, selectedMoment }) => {
  ...
  const { updateLoading } = useSelector((state) => state.moment);
  const { message } = useSelector((state) => state.momentTag);
  const { showEditMomentModel, setShowEditMomentModel } =
    useContext(GlobalContext);
  const [show, setShow] = useState("FORM"); // FORM, COVER, TAG_PEOPLE
  const [title, setTitle] = useState("Edit your Moment");

  // form fields
  const [caption, setCaption] = useState(selectedMoment.caption || "");
  const [whoCanWatch, setWhoCanWatch] = useState(
    selectedMoment.whoCanWatch.toString() || "0"
  );
  const [whoCanMessage, setWhoCanMessage] = useState(
    selectedMoment.whoCanMessage.toString() || "0"
  );
  const [videoSize, setVideoSize] = useState(selectedMoment?.fileSize || null);
  const [videoUrl, setVideoUrl] = useState(selectedMoment.videoUrl || null);
  const [videoLength, setVideoLength] = useState(
    selectedMoment?.videoLength || null
  );
  const [cover, setCover] = useState({
    url: selectedMoment.coverImgUrl || null,
    id: null,
  });
  const [categoryId, setCategoryId] = useState(
    selectedMoment.categoryId || null
  );
  const [hashTags, sethashTags] = useState(selectedMoment?.hashTags || []);
  ...
  useEffect(() => {
    dispatch(getTaggedUsersInMoment(selectedMoment?.id));
  }, [selectedMoment, message]);
  ...
  const handleUploadMoment = async () => {
    ...
    dispatch(updateMoment(selectedMoment.id, obj));
  };
  ...
  return (
    <EditMomentFormStyled
      className={`${showEditMomentModel && "ShowEditMomentForm"}`}
    >
        ...
        <div className="form">
          <Row className="form__row">
            ...
            <Col md={8} sm={8} xs={8} className="form__item">
              <div className="form__right">
                <label htmlFor="editMoment" className="form__card">
                  <div className="left">
                    <img src={UploadIcon} alt="" className="icon" /> Edit Moment
                  </div>
                  <div className="right">
          ...
          <div className="form__submit">
            <button onClick={handleUploadMoment}>
              {updateLoading ? "Updating Moment..." : "Update Moment"}
            </button>
          </div>
        </div>
      )}
      ...
      {show === "TAG_PEOPLE" && <TagPeople momentId={selectedMoment.id} />}
    </EditMomentFormStyled>
  );
};
...
const EditMomentFormStyled = styled.div`
  height: 100vh;
  width: 100%;
